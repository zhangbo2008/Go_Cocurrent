...
天生支持并发.  channel: 线程之间通讯.

集成. k8s.


内核----起的进程数量-----xiancheng ===携程==
*/
...
func (w *Worker) Start() {
	go func() {
		...
		for {
			// regist current job channel to worker pool  // debug技巧: 看channel里面的qcount 里面的数量就是channel里面现在存在的东西数量.  // 注册.
