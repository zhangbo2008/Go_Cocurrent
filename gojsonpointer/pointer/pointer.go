//NewJsonPointer// Copyright 2015 xeipuuv ( https://github.com/xeipuuv )
//
...
// 根据路径顺着走即可.
	for ti, token := range p.referenceTokens {
		...
		previousNodes[ti] = node  //上来进入初始节点.
		...
		case map[string]interface{}:
			...
			if _, ok := v[decodedToken]; ok {
				node = v[decodedToken] //找到了key 就进入他. 更新node
