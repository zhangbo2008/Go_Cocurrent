...
import (
	"awesomeProject8/gojsonpointer-master/pointer"
	"encoding/json"
	"testing"
)
...
func TestEscaping(t *testing.T) {
	...
	for i := range ins {
		...
		p, err := pointer.NewJsonPointer(ins[i])
		if err != nil {
...
func BenchmarkParse(b *testing.B) {
	for i := 0; i < b.N; i++ {
		pointer.NewJsonPointer(`/definitions/simple/0/next`)
	}
}

func BenchmarkParseWithEscape(b *testing.B) {
	for i := 0; i < b.N; i++ {
		pointer.NewJsonPointer(`/definiti~0ons/simple/0/next`)
	}
}

func BenchmarkString(b *testing.B) {
	p, _ := pointer.NewJsonPointer(`/definitions/simple/0/next`)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		p.String()
	}
...
func BenchmarkStringWithEscape(b *testing.B) {
	p, _ := pointer.NewJsonPointer(`/definiti~0ons/simple/0/next`)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		p.String()
	}
...
func TestFullDocument(t *testing.T) {
	...
	p, err := pointer.NewJsonPointer(in)
	if err != nil {
...
func BenchmarkGet(b *testing.B) {
	p, _ := pointer.NewJsonPointer(`/obj/d/1/f`)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
