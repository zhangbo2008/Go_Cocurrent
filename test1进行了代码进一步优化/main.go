...
做一个自己优化试试, worker里面不要workerpool了
让所有的worker都共享一个workerpool
...
func (w *Worker) Start() {
	go func() {
		...
		for {
			...
			WorkerPool <- w.JobChannel     //抽象含义表示这个channel已经用完了,这个worker又可以回到pool中了.
...
func NewDispatcher(maxWorkers int) *Dispatcher {  //go没法传默认参数,曹乐!
	...
	return &Dispatcher{WorkerPool: WorkerPool}//这里面简历的workerpool表示大家共用的了.  //直接都调用最原始的那个.
}
